...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    hidden(div(id = "step3",
               ...
               fluidRow(
                 column(12,
                        p(textOutput("locked_info", inline = TRUE), "Här är det färdiga receptet för en färgpasta med önskad mängd färgande pigment. Den utgör basen för ett komplett färgsystem för linoljemålning."),
                        hr()
                        ),
               ),
               fluidRow(
                 column(6,
                        box(title = "Recept", status = "danger", solidHeader = TRUE, width = 12,
                            ...
                            p("Kritiskt oljetalet = minsta möjliga mängd som krävs för att väta alla pigmenten. För enklare blandning med färgblandare i borrmaskin och bra strykbarhet, välj 1,6–2,2×."),
                            sliderInput("oil_multiplier", "Faktor på kritiskt oljetal",
                                        min = 1.0, max = 2.5, value = 1.6, step = 0.1),
                 ...
                 column(6,
                        box(title = "Färginfo", status = "primary", solidHeader = TRUE, width = 12,
               ...
               fluidRow(
                 column(12,
                        ...
                        p("Häll upp den kokta linoljan i ett kärl (gärna en plåtburk för färg. Väg upp och tillsätt pigmenten. Det kan verka förvånande att det räcker med så lite linolja i botten av kärlet, men ha tålamod och låt pigmenten vila, gärna över natten; Pigmenten väts med lite tid med hjälp av oljans kapillärvandring. Riv sedan blandningen noga med färgblandare i borrmaskin."),
                        h4("Instruktion för målning"),
                        p("Grundfärgsstrykning: Pastan kan målas/gnuggas in med påstrykare direkt, som en mager grundfärg. Lägg till den mängd kokt linolja som gör pastan smidig att påföra underlaget med."),
                        p("Mellanstrykning: För mellanstrykningen, tillför ytterligare kokt linolja, utifrån principen ”fett över magert”, i precis den mängd, men inte heller mindre, som gör att färgen utstruken på en glasbit fortfarande förblir ogenomskinlig."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$to_step3, {
    ...
    for (id in names(color_g)) kritisk_oil <- kritisk_oil + color_g[id] * km[[id]]$oil / 100
    kritisk_oil <- round(kritisk_oil, 1)
    
    final_data(list(
      zn = zn_g, ti = ti_g, color = color_g,
  ...
  output$final_recipe <- renderTable({
    ...
    df <- data.frame(Ingrediens = "Kallpressad linolja", Gram = olja, stringsAsFactors = FALSE)
  ...
  output$paint_name_title <- renderText({ if(input$paint_name != "") paste("–", input$paint_name) })
  ...
  output$download <- downloadHandler(
    ...
    content = function(file) {
      r <- final_data()
      lines <- c(
        paste("# Paint-o-matic recept –", if(input$paint_name != "") input$paint_name else "Ej namngiven färg"),
        "",
        paste(Sys.Date()),
        paste("Total pigmentvikt:", r$total_pigment, "g"),
        paste("Zinkvit i vitbas:", input$zinc_ratio, "%"),
        paste("Linolja:", vald_olja(), "g (", input$oil_multiplier, "× kritiskt oljetal)"),
        paste("Hex-kod:", r$hex),
        "", "## Recept:",
