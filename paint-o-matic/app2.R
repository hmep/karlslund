# Paint-o-matic – SLUTGILTIG VERSION med ny bättre Steg 2
# Direkt procent av totalvikt + vitbas fylls automatiskt
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tags$head(tags$style(HTML("
      ...
      .big-btn { font-size: 22px; padding: 15px 40px; }
    "))),
    ...
    # STEG 2 – NY BÄTTRE VERSION
    hidden(div(id = "step2",
               h2("Steg 2 – Blanda färg"),
               fluidRow(
                 column(12,
                        p(textOutput("locked_info", inline = TRUE)),
                        p("Ange exakt hur många procent varje pigment ska vara av den totala pigmentvikten. Vitbasen fylls automatiskt upp till 100 %. Om den samlade mängde pigment överskrider 100 % normaliseras mängderna till 100 % i nästa steg."),
                        br()
                 ),
               ),
               ...
               fluidRow(
                 column(6,
                        ...
                        conditionalPanel("input.p1 != ''", 
                                         numericInput("pct1", "Procent av totalen (%)", value = 0, min = 0, max = 100, step = 1)
                        ),
                        selectInput("p2", "Pigment 2", c("Inget" = "", color_choices)),
                        conditionalPanel("input.p2 != ''", 
                                         numericInput("pct2", "Procent av totalen (%)", value = 0, min = 0, max = 100, step = 1)
                        ),
                        selectInput("p3", "Pigment 3", c("Inget" = "", color_choices)),
                        conditionalPanel("input.p3 != ''", 
                                         numericInput("pct3", "Procent av totalen (%)", value = 0, min = 0, max = 100, step = 1)
                        ),
                        actionButton("reset_pigments", "Nollställ alla pigment", class = "btn-default"),
                        br(), br(),
    ...
    hidden(div(id = "step3",
               ...
               fluidRow(
                 column(12,
                        p(textOutput("locked_info", inline = TRUE), "Här är det färdiga receptet för en färgpasta med önskad mängd färgande pigment. Den utgör basen för ett komplett ”färgsystem”."),
                        h4("Instruktion för rivning (blandning)"),
                        p("Pigmenten läggs att väta i ett kärl (gärna en plåtburk för färg) med linolja över natten. Det kan verka förvånande att det fungerar med så lite linolja -- ha dock tålamod och låt pigmenten vila. Om du ändå skulle vilja lägga till mer kokt linolja går det bra, upp till dubbla mängden. Därefter rivs blandningen noga med färgblandare i borrmaskin."),
                        h4("Instruktion för målning"),
                        p("Grundfärgsstrykning: Pastan kan målas/gnuggas in med påstrykare direkt, som en mager grundfärg. Lägg till den mängd kokt linolja, men inte mer, som gör pastan möjlig att med."),
                        p("Mellanstrykning: För mellanstrykningen, tillför ytterligare kokt linolja, utifrån principen ”fett över magert”, i precis den mängd (men inte heller mindre) som gör att färgen utstruken på en glasbit förblir ogenomskinlig."),
                        p("Slutstrykning: För ökad glans och hållbarhet kan sista strykningen med moddlare därutöver med fördel innehålla upp till 5 % soloxiderad linolja."),
                        br()
                 ),
               ),
               fluidRow(
                 column(6,
                        box(title = "Recept", status = "danger", solidHeader = TRUE, width = 12,
                 ...
                 column(6,
                        box(title = "Färg", status = "primary", solidHeader = TRUE, width = 12,
                            uiOutput("final_preview"),
                            br(),
                            tags$b("Hex-kod: "), textOutput("final_hex")
                        )
                 )
               ),
               br(),
...
server <- function(input, output, session) {
  ...
  # Visa Steg 1 direkt när appen startar
  observe({ showElement("step1") })
  
  # Reaktiv lagring av slutligt recept
  ...
  # Tillbaka-knappar
  observeEvent(input$back_to_step1, { hide("step2"); show("step1") })
  observeEvent(input$back_to_step2, { final_data(NULL); hide("step3"); show("step2") })
  observeEvent(input$restart, { final_data(NULL); hide("step3"); hide("step2"); show("step1") })
  
  # Nollställ pigment i Steg 2
  ...
  current_pcts <- reactive({
    ...
    list(p1 = p1, p2 = p2, p3 = p3, white = white, total_used = used)
  })
  
  output$summary_pct <- renderText({
    x <- current_pcts()
    paste0("Pigment 1: ", x$p1, "%   |   Pigment 2: ", x$p2, "%   |   Pigment 3: ", x$p3, "%   |   Vitbas: ", x$white, "%")
  })
  ...
  live_color <- reactive({
    ...
    raw_color <- numeric()
    if (input$p1 != "" && p$p1 > 0) raw_color[input$p1] <- total_g * p$p1 / 100
    if (input$p2 != "" && p$p2 > 0) raw_color[input$p2] <- total_g * p$p2 / 100
    if (input$p3 != "" && p$p3 > 0) raw_color[input$p3] <- total_g * p$p3 / 100
    ...
    list(hex = hex, raw_zn = zn_g, raw_ti = ti_g, raw_color = raw_color)
  })
  ...
  # Steg 3 – resultat
  output$final_preview <- renderUI({ req(final_data()); tags$div(class = "preview-box", style = paste0("background:", final_data()$hex)) })
  output$final_hex <- renderText({ req(final_data()); final_data()$hex })
  output$exact_total <- renderText({ req(final_data()); paste0("Total pigmentvikt: ", final_data()$total_pigment, " g") })
  
  output$final_recipe <- renderTable({
    req(final_data())
    r <- final_data()
    df <- data.frame(Ingrediens = "Kallpressad linolja", Gram = r$oil, stringsAsFactors = FALSE)
    if (r$zn > 0.1) df <- rbind(df, c("Zinkvit PW4 (#44100)", r$zn))
    if (r$ti > 0.1) df <- rbind(df, c("Titanvit PW6 (#44400)", r$ti))
    for (id in names(r$color)) {
      name <- km[[id]]$name
      df <- rbind(df, c(paste0(name, " (#", id, ")"), r$color[id]))
    }
    df
  ...
  output$download <- downloadHandler(
    filename = function() paste0("Paint-o-matic_", if(input$paint_name != "") input$paint_name else "recept", "_", format(Sys.time(), "%Y%m%d_%H%M"), ".txt"),
    content = function(file) {
      r <- final_data()
      lines <- c(
        paste("# Paint-o-matic recept –", if(input$paint_name != "") input$paint_name else "Okänd färg"),
        paste("#", Sys.Date(), Sys.time()),
        paste("Total pigmentvikt:", r$total_pigment, "g"),
        paste("Zinkvit i vitbas:", input$zinc_ratio, "%"),
        paste("Hex-kod:", r$hex),
        "", "RECEPT:",
        paste("- Kallpressad linolja:", r$oil, "g")
      )
      if (r$zn > 0.1) lines <- c(lines, paste("- Zinkvit PW4:", r$zn, "g"))
      if (r$ti > 0.1) lines <- c(lines, paste("- Titanvit PW6:", r$ti, "g"))
      for (id in names(r$color)) lines <- c(lines, paste("- ", km[[id]]$name, ":", r$color[id], "g"))
